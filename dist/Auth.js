"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var formidable_1 = __importDefault(require("formidable"));
var bcrypt_1 = __importDefault(require("bcrypt"));
var AuthController = /** @class */ (function () {
    function AuthController(userModel, userSessions, hashRounds, mailTransporterUser, mailTransporterPass, passwordLength) {
        this.userModel = userModel;
        this.userSession = userSessions;
        this.hashRounds = hashRounds || 15;
        this.mailTransporterUser = mailTransporterUser;
        this.mailTransporterPass = mailTransporterPass;
        this.passwordLength = passwordLength || 8;
    }
    AuthController.prototype.SignUp = function (request, response) {
        var _this = this;
        var form = new formidable_1.default.IncomingForm();
        try {
            form.parse(request, function (error, fields, files) { return __awaiter(_this, void 0, void 0, function () {
                var username, email, password, verifiedPassword, isUserAlreadyExisting, salt, hashedPassword, newUser, savedUser;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (error) {
                                return [2 /*return*/, response
                                        .status(500)
                                        .json({ msg: "Network Error: Failed to sign user up" })];
                            }
                            username = fields.username, email = fields.email, password = fields.password, verifiedPassword = fields.verifiedPassword;
                            if (!username || !email || !password || !verifiedPassword) {
                                return [2 /*return*/, response.status(400).json({ msg: "All fields are required" })];
                            }
                            if (password.length < this.passwordLength) {
                                return [2 /*return*/, response.status(400).json({
                                        msg: "Password has to be at least " + this.passwordLength + " characters long",
                                    })];
                            }
                            if (password !== verifiedPassword) {
                                return [2 /*return*/, response.status(400).json({ msg: "Password's do not match" })];
                            }
                            return [4 /*yield*/, this.userModel.findOne({
                                    email: email,
                                })];
                        case 1:
                            isUserAlreadyExisting = _a.sent();
                            if (!isUserAlreadyExisting) {
                                return [2 /*return*/, response
                                        .status(400)
                                        .json({ msg: "Account with this email already exist" })];
                            }
                            return [4 /*yield*/, bcrypt_1.default.genSalt(this.hashRounds)];
                        case 2:
                            salt = _a.sent();
                            return [4 /*yield*/, bcrypt_1.default.hash(password, salt)];
                        case 3:
                            hashedPassword = _a.sent();
                            newUser = new this.userModel({
                                username: username,
                                email: email,
                                password: hashedPassword,
                            });
                            return [4 /*yield*/, newUser.save()];
                        case 4:
                            savedUser = _a.sent();
                            return [2 /*return*/];
                    }
                });
            }); });
            return response.status(201).json({ msg: "Account successfully created" });
        }
        catch (error) {
            console.error(error);
            return response
                .status(500)
                .json({ msg: "Network Error: Failed to signup user" });
        }
    };
    AuthController.prototype.Login = function (request, response) {
        var _this = this;
        var form = new formidable_1.default.IncomingForm();
        try {
            form.parse(request, function (error, fields, files) { return __awaiter(_this, void 0, void 0, function () {
                var email, password, user, userSession;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (error) {
                                return [2 /*return*/, response
                                        .status(500)
                                        .json({ msg: "Network Error: Failed to login in user" })];
                            }
                            email = fields.email, password = fields.password;
                            if (!email || !password) {
                                return [2 /*return*/, response.status(400).json({ msg: "All fields are required" })];
                            }
                            return [4 /*yield*/, this.userModel.findOne({ email: email })];
                        case 1:
                            user = _a.sent();
                            if (!user) {
                                return [2 /*return*/, response
                                        .status(404)
                                        .json({ msg: "Account with this email does not exist" })];
                            }
                            userSession = {
                                email: email,
                                id: user._id,
                            };
                            return [2 /*return*/, userSession];
                    }
                });
            }); });
            return {};
        }
        catch (error) {
            console.error(error);
            return response
                .status(500)
                .json({ msg: "Network Error: Failed to login user" });
        }
    };
    return AuthController;
}());
exports.default = AuthController;
//# sourceMappingURL=Auth.js.map